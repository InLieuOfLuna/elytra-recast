...
object ElytraHelper {
    fun castElytra(player: ClientPlayerEntity): Boolean {
        return if (checkElytra(player) && checkFallFlyingIgnoreGround(player)) {
            ...
            networkHandler?.sendPacket(ClientCommandC2SPacket(player, ClientCommandC2SPacket.Mode.START_FALL_FLYING))
            true
        } else false
    }

    private fun checkElytra(player: ClientPlayerEntity): Boolean {
        return if (player.input.jumping && !player.abilities.flying && !player.hasVehicle() && !player.isClimbing) {
